...
for ch in ['1', '2', '3', '4']:
    ...
    for i, vc in enumerate(vc_list):
        ...
        # 1. Look for the exact paragraph number at the start of a line
        pattern = r'(?m)^\s*(?:Para\s*)?' + re.escape(para_num) + r'\b[\.\-\)]\s*'
        m = re.search(pattern, search_window, re.IGNORECASE)
        
        # 2. If not found, look for just the number and the first word of the title
        if not m:
    ...
    # Interpolate missing indices strictly
