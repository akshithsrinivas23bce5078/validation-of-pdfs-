...
for ch in ['1', '2', '3', '4']:
    ...
    while i < len(indices) - 1:
        j = i
        while j < len(indices) - 1 and indices[j] == indices[j+1]:
            j += 1
        if j > i:
            k = j + 1
            while k < len(indices) and indices[k] == indices[i]:
                k += 1
            if k < len(indices):
                next_val = indices[k]
                total_space = next_val - indices[i]
                num_squeezed = j - i + 1
                step = total_space // (num_squeezed + 1)
                for step_idx, x in enumerate(range(i, j + 1)):
                    indices[x] = indices[i] + step * step_idx
        i = j + 1
