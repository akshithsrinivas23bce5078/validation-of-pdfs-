...
for ch in ['1', '2', '3', '4']:
    ...
    for i, vc in enumerate(vc_list):
        ...
        if idx_in_window != -1:
            ...
            # Fall back to exact expected page
    ...
    # Fix non-monotonicity (always keep it monotonically increasing)
    # If indices[i] < indices[i-1], it means either a false positive early, or the fallback is weird.
    # We will force indices[i] = indices[i-1] to prevent text overlapping backwards.
...
# Check Paragaphs
for c in val_chunks:
    if str(c['chapter']) == '1' and c.get('para') in [4, 6]:
        print(f"\n--- {c['heading']} ---")
        print(c['text'][:150])
