...
for ch in ['1', '2', '3', '4']:
    ...
    for i, vc in enumerate(vc_list):
        ...
        words = [re.escape(w) for w in re.split(r'\W+', title.strip()) if len(w) >= 3]
        
        m = None
        if len(words) >= 1:
            first_few_words = r'\W+'.join(words[:2])
            # PERMISSIVE REGEX: allows space after number without punctuation!
            pattern = r'(?m)^\s*(?:Para\s*)?' + re.escape(para_num) + r'\b(?:[\.\-\)]\s*|\s+)(?:.{0,30}?)' + first_few_words
